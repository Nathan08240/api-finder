...
const UploadFile: React.FC<UploadFileProps> = ({ activeTab, setActiveTab, calculateStorageMo }) => {
  ...
  return (
    <>
      <ListItem
        ...
        disabled={activeTab != "bibliotheque" || calculateStorageMo >= 1000 ? true : false}
        onClick={() => {
      ...
      <Modal open={open} onClose={handleClose}>
        <Box sx={style}>
          <Typography>Importer un fichier (espace disponible : {storageAvailable} Mo)</Typography>
          {filesArray.length > 0 && (
            <>
              <List>
                <DisplayList />
